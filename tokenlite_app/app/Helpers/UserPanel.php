...
class UserPanel
{
    ...
    public static function user_balance_card($data = null, $atttr = '')
    {
        ...
        $base_out = '<li class="token-balance-sub"><span class="lead">' . ($base_con > 0 ? $base_con : '0') . '</span><span class="sub">' . strtoupper($base_cur) . '</span></li>';

        $cur1_out = $cur2_out = '';
        if(gws('user_in_cur1', 'eth')!='hide') {
            $cur1 = gws('user_in_cur1', 'eth');
            $cur1_con = (gws('pmc_active_'.$cur1) == 1) ? to_num($data->$cur1, 'auto') : 0;
            $cur1_out = ($cur1 != $base_cur) ? '<li class="token-balance-sub"><span class="lead">' . ($cur1_con > 0 ? $cur1_con : '0') . '</span><span class="sub">' . strtoupper($cur1) . '</span></li>' : '';
        }

        if(gws('user_in_cur2', 'btc')!='hide') {
            $cur2 = gws('user_in_cur2', 'btc');
            $cur2_con = (gws('pmc_active_'.$cur2) == 1) ? to_num($data->$cur2, 'auto') : 0;
            $cur2_out = ($cur2 != $base_cur) ? '<li class="token-balance-sub"><span class="lead">' . ($cur2_con > 0 ? $cur2_con : '0') . '</span><span class="sub">' . strtoupper($cur2) . '</span></li>' : '';
        }
    ...
    public static function token_sales_progress($data = null, $atttr = '')
    {
        ...
        $sales_state = '';
        if (!hide_opt_count()) {
            $sales_end_in .= '<span class="card-sub-title ucap mgb-0-5x">'.__('Stage '.gws('actived_stage').'  End in').'</span><div class="countdown-clock" data-date="'._date(active_stage()->end_date, 'Y/m/d H:i:s').'"></div>';
            $sales_start_in .= '<span class="card-sub-title ucap mgb-0-5x">'.__('Stage '.gws('actived_stage').' Start in').'</span><div class="countdown-clock" data-date="'._date(active_stage()->start_date, 'Y/m/d H:i:s').'"></div>';
